...
public class PullDownScrollView extends LinearLayout {
    ...
    private RotateAnimation reverseAnimation;   //动画
    ...
    private void init() {   //动画初始化
        animation = new RotateAnimation(0, -180,
                RotateAnimation.RELATIVE_TO_SELF, 0.5f,
                RotateAnimation.RELATIVE_TO_SELF, 0.5f);
        animation.setInterpolator(new LinearInterpolator());
        animation.setDuration(250);
        animation.setFillAfter(true);         //箭头旋转的动画
     ...
     */
    @Override   //重写，是onTouchEvent的预处理，返回true表示已经消费了这个动作，不会再用到ontouchevent,否则还会在ontouchevent中处理
    ...
    private void handleHeadElastic(MotionEvent event) {    //具体的处理过程
        if (refreshListener != null && mElastic != null) {
            ...
            case MotionEvent.ACTION_MOVE:
                //Log.i(TAG, "move");
                int tempY = (int) event.getY();    //现在的纵坐标值
                ...
                if (state != IPullDownElastic.REFRESHING && isElastic) {               
                    ...
                    if (state == IPullDownElastic.DONE) {   //如果现在的状态是已完成
                        if (tempY - startY > 0) {
                            state = IPullDownElastic.PULL_To_REFRESH;
                            changeHeaderViewByState(state, false);
                        }
                    ...
                    if (tempY - startY > 0) {
                        setMargin((tempY - startY)*2/3 + refreshTargetTop);
                    }
                }
                break;
            }
    ...
    private void changeHeaderViewByState(int state, boolean isBack) {
        ...
        case IPullDownElastic.REFRESHING:
            mElastic.showArrow(View.GONE);   //箭头
            mElastic.showProgressBar(View.VISIBLE);   //圆形进度条
            mElastic.showLastUpdate(View.GONE);   //上次更新
            mElastic.setTips(note_refreshing);     //提示语
...
//        }
    ...
    public void finishRefresh(String text) {
        ...
        state = IPullDownElastic.DONE;
        mElastic.setLastUpdateText(text);   //设置text显示传入的上次更新时间
        changeHeaderViewByState(state,false);
...
//        invalidate();
    ...
    private boolean canScroll() {   //判断是否能下拉，如果子view的头部不可见，则不能下拉
        View childView;
        if (getChildCount() > 1) {
            childView = this.getChildAt(1);
            if (childView instanceof AbsListView) {
                int top = ((AbsListView) childView).getChildAt(0).getTop();   //容易出现空指针异常
